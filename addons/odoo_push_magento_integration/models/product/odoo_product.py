...
class MagentoSyncOdoo(models.Model):
    ...
    @api.model
    def create(self, values): # 谁去调用这个函数？
        # check unique code
        ...
        if res.on_magento and 'm2_info_attr_set' not in values and 'm2_info_cancel' not in values:
            ...
            if magento and res.default_code:
                ...
                if product:
                    ...
                    if res.image_1024:
                        image = res.image_1024.replace(b"\n", b"").decode('ascii')
                        mimetype = self._get_image_type(res.image_1024)
        ...
        if 'm2_info_cancel' not in values:
            ...
            if self.magento_ok:
                if on_magento:
                    ...
                    if 'on_magento' in onchange:
                        ...
                        if 'image_medium' in onchange or 'product_image_ids' in onchange: ##TODO
                            check += 1
                            if self.image_1024 == False:
                                body['product'].update({
                                    "media_gallery_entries": []})
                            else:
                                image = self.image_1024.replace(b"\n", b"").decode('ascii')
                                mimetype = self._get_image_type(self.image_1024)
            ...
            if not self.magento_ok:
                if 'on_magento' in onchange:
                    ...
                    if len(self.product_template_image_ids) > 0: #没有这
                        if not self.image_1024:
                            body['product'].update({
                                "media_gallery_entries": []})
                        count = 1
                        for img in self.product_template_image_ids:
                            ...
                            if img.image:
                                extra_image = img.image.replace(b"\n", b"").decode('ascii')
                                extra_mimetype = self._get_image_type(self.image_1024)
                                # extra_decode = base64.b64decode(img.image)
                                # extra_mimetype = guess_mimetype(extra_decode, default='image/png')
                                # if extra_mimetype not in ('image/jpeg', 'image/png', 'image/gif'):
                                #     raise Warning(_('Image Format Not Support On Magento '))
