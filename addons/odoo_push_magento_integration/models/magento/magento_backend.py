...
class MagentoBackend(models.Model):
    ...
    _logger.info("MagentoBackend class init")
    print("MagentoBackend class init")

    @api.model
    def create(self, values):
        record = super(MagentoBackend, self).create(values)
        _logger.info("magento backen create, %s", values.get('web_url'))
        print("magento backen create")
        print(values.get('web_url'))
    ...
    def fetch_attribute_set(self):
        # get from config
        # backend_id = self.id
        _logger.info("fetch_attribute_set.....")
        print("fetch_attribute_set.....")
        url = self.web_url
        ...
        _logger.info("fetch_attribute_set, firt_pull %s ", firt_pull)
        print("fetch_attribute_set end, %s", firt_pull)
        if product:
            try:
                product_attr_sets = product.list_attribute_set(condition='0')
            except ValueError as e:
                _logger.error("fetch_attribute_set fetch fail %s", e)
            ...
            if len(product_attr_sets['items']) > 0:
                ...
                if firt_pull != 0:
                    ...
                    for item in product_attr_sets['items']:
                        try:
                            print("create new AttributeSet", item['attribute_set_name'])
                            AttributeSet.create({
                                'name': item['attribute_set_name'],
                                'attribute_set_id': item['attribute_set_id'],
                                'attribute_set_name': item['attribute_set_name'],
                            ...
                            print("incorrect values %s ", e)
                            raise ValueError(_("Some %s contains incorrect values.") % e)

        else:
            _logger.warning("fetch_attribute_set fail, invalid product")
            print("fetch_attribute_set fail, invalid product")
